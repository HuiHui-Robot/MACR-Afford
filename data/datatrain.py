...
class TrainData(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        aff_name, object_name = names[-3], names[-2]
        exocentric_image = self.load_img(exocentric_image_path)
        aff_label = self.aff_list.index(aff_name)

        ego_path = os.path.join(self.egocentric_root, aff_name, object_name)
        ...
        aff_name = aff_name.replace('_', ' ')
        aff_object_name = f"{object_name}-{aff_name}"
