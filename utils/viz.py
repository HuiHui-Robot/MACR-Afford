...
def viz_pred_test(args, image, ego_pred, GT_mask, aff_list, aff_label, img_name, epoch=None):
    ...
    temp = 'WSMA_Unseen'
    os.makedirs(os.path.join(args.save_path, temp, 'viz_test'), exist_ok=True)
    if epoch:
        fig_name = os.path.join(args.save_path, temp, 'viz_test', "epoch" + str(epoch) + '_' + img_name + '.jpg')
    else:
        fig_name = os.path.join(args.save_path, temp, 'viz_test', img_name + '.jpg')

    ego_pred.save(fig_name)
    plt.close()
